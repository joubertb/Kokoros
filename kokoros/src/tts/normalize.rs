...
lazy_static! {
    ...
    static ref ETC_CAPITAL_RE: Regex = Regex::new(r"\betc\. ([A-Z])").unwrap();
...
pub fn normalize_text(text: &str) -> String {
    ...
    text = NEWLINE_SPACE_RE.replace_all(&text, "\n\n").to_string();
    ...
    text = MRS_RE.replace_all(&text, "Mrs $1").to_string();
    // Only replace "etc." if not followed by a capital letter (which would indicate start of sentence)
    text = ETC_CAPITAL_RE.replace_all(&text, "etc. $1").to_string();
